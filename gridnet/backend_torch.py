...
def inner_step_fn(
    ...
    input_index_tensor = compute_input_indices(block_size, device)
    ...
    def inner_fn(
        ...
        results = apply_activation(activation, results)
...
def gated_gridnet_step_pytorch(
    weight: torch.Tensor,
    bias: torch.Tensor,
    init_activations: torch.Tensor,
    inner_iterations: int,
    ...
    block_fn = gated_inner_step_fn(block_size, weight.device, activation)
    output_blocks = []
    padded_init_acts = F.pad(init_activations, (1,) * 6)
    for a in range(1, m + 1, block_size):
        for b in range(1, n + 1, block_size):
...
def gated_inner_step_fn(
    ...
    def inner_fn(
        ...
        pre_act = (patches * weight[:, 0].flatten(1).t()).sum(-1) + bias[0].flatten()
        gate_pre_act = (patches * weight[:, 1].flatten(1).t()).sum(-1) + bias[
            1
        ].flatten()
        ...
        out[:, 1:-1, 1:-1, 1:-1] = results
        return out

    return inner_fn


def apply_activation(activation: ActivationFn, x: torch.Tensor) -> torch.Tensor:
    if activation == "silu":
        return F.silu(x)
...
def compute_input_indices(block_size: int, device: torch.device) -> torch.Tensor:
