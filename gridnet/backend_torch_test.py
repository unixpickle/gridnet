...
def test_gridnet_step_zero_iters(shape: Tuple[int, int, int], block_size: int):
    ...
    scales = torch.ones(*shape)
    outputs = gridnet_step_pytorch(
        weights, biases, scales, inputs, 0, block_size=block_size
    )
...
def test_gridnet_step_zero_weights(shape: Tuple[int, int, int], block_size: int):
    ...
    scales = torch.ones(*shape) * 0.31
    outputs = gridnet_step_pytorch(
        weights, biases, scales, inputs, 10, block_size=block_size
    )
...
def test_forward_benchmark(benchmark):
    ...
    def fn():
        gridnet_step_pytorch(weights, biases, scales, inputs, 10, block_size=8)
        torch.cuda.synchronize()
...
def test_backward_benchmark(benchmark):
    ...
    def fn():
        out = gridnet_step_pytorch(weights, biases, scales, inputs, 10, block_size=8)
