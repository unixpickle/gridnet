...
template <typename scalar_t>
__device__ __forceinline__ uint load_activations(
    ...
    scalar_t eps)
{
    ...
    uint activationsSize = load_activations(
        activations,
        initActivations,
        threadOffset,
        numThreads,
...
__global__ void __launch_bounds__(512, 1) gated_gridnet_cuda_forward_kernel(
    const torch::PackedTensorAccessor32<scalar_t, 5> weight,
    const torch::PackedTensorAccessor32<scalar_t, 4> bias,
    const torch::PackedTensorAccessor32<scalar_t, 4> initActivations,
    torch::PackedTensorAccessor32<scalar_t, 4> outActivations,
    uint innerIterations)
{
    ...
    // The weights should be loaded directly into registers.
    scalar_t localWeights[27];
    scalar_t localGateWeights[27];
    for (uint i = 0; i < 27; i++) {
        localWeights[i] = weight[i][0][blockZ * blockSize + threadIdx.z][blockY * blockSize + threadIdx.y][blockX * blockSize + threadIdx.x];
        localGateWeights[i] = weight[i][1][blockZ * blockSize + threadIdx.z][blockY * blockSize + threadIdx.y][blockX * blockSize + threadIdx.x];
    }
    scalar_t localBias = bias[0][blockZ * blockSize + threadIdx.z][blockY * blockSize + threadIdx.y][blockX * blockSize + threadIdx.x];
    scalar_t localGateBias = bias[1][blockZ * blockSize + threadIdx.z][blockY * blockSize + threadIdx.y][blockX * blockSize + threadIdx.x];
    ...
    for (uint i = 0; i < innerIterations; i++) {
        ...
        scalar_t actDot = localBias;
        scalar_t gateDot = localGateBias;
        for (uint a = 0; a < 3; a++) {
            for (uint b = 0; b < 3; b++) {
                for (uint c = 0; c < 3; c++) {
                    uint cellOffset = threadIdx.x + c + (blockSize + 2) * (threadIdx.y + b + (blockSize + 2) * (threadIdx.z + a));
                    scalar_t act = activations[cellOffset];
                    actDot += act * localWeights[a * 9 + b * 3 + c];
                    gateDot += act * localGateWeights[a * 9 + b * 3 + c];
                }
            }
        }

        scalar_t prevAct = activations[paddedOffset];
        scalar_t gateValue = sigmoid(gateDot);
        scalar_t output = (gateValue * prevAct +
                           (1 - gateValue) * Activation<Act, scalar_t>::forward(actDot));
...
template <typename scalar_t, typename Act, uint iterationsBufferSize>
__global__ void __launch_bounds__(512, 1) gated_gridnet_cuda_backward_kernel(
    const torch::PackedTensorAccessor32<scalar_t, 5> weight,
    const torch::PackedTensorAccessor32<scalar_t, 4> bias,
    const torch::PackedTensorAccessor32<scalar_t, 4> initActivations,
    const torch::PackedTensorAccessor32<scalar_t, 4> outGrads,
    torch::PackedTensorAccessor32<scalar_t, 5> weightGradOut,
    torch::PackedTensorAccessor32<scalar_t, 4> biasGradOut,
    torch::PackedTensorAccessor32<scalar_t, 4> activationsGradOut,
    uint innerIterations)
{
    // A tensor of size (blockSize + 2) ^ 3
    ...
    scalar_t localGateWeights[27];
    scalar_t localWeightsGradAcc[27];
    scalar_t localGateWeightsGradAcc[27];
    for (uint i = 0; i < 27; i++) {
        localWeights[i] = weight[i][0][blockZ * blockSize + threadIdx.z][blockY * blockSize + threadIdx.y][blockX * blockSize + threadIdx.x];
        localGateWeights[i] = weight[i][1][blockZ * blockSize + threadIdx.z][blockY * blockSize + threadIdx.y][blockX * blockSize + threadIdx.x];
        localWeightsGradAcc[i] = 0;
        localGateWeightsGradAcc[i] = 0;
    }
    scalar_t localBias = bias[0][blockZ * blockSize + threadIdx.z][blockY * blockSize + threadIdx.y][blockX * blockSize + threadIdx.x];
    scalar_t localBiasGradAcc = 0.0;
    scalar_t localGateBias = bias[1][blockZ * blockSize + threadIdx.z][blockY * blockSize + threadIdx.y][blockX * blockSize + threadIdx.x];
    scalar_t localGateBiasGradAcc = 0.0;
    ...
    for (uint i = 0; i < innerIterations - 1; i++) {
        ...
        scalar_t gateValue = sigmoid(gateDot);
        scalar_t output = (gateValue * localAct +
                           (1 - gateValue) * Activation<Act, scalar_t>::forward(actDot));
    ...
    __syncthreads(); // only necessary in the innerIterations==1 case
    activationsBuffer[innerIterations - 1] = activations[paddedOffset];
    __syncthreads();
    ...
    for (int i = innerIterations - 1; i >= 0; i--) {
        ...
        // Recompute the input to the activations.
        scalar_t actDot = localBias;
        scalar_t gateDot = localGateBias;
        for (uint a = 0; a < 3; a++) {
            for (uint b = 0; b < 3; b++) {
        ...
        scalar_t localAct = activations[paddedOffset];
        scalar_t gateValue = sigmoid(gateDot);
        scalar_t actDotGrad = outGrad * (1 - gateValue) * Activation<Act, scalar_t>::backward(actDot);
        scalar_t actOut = Activation<Act, scalar_t>::forward(actDot);
        scalar_t gateDotGrad = outGrad * gateValue * (1 - gateValue) * (localAct - actOut);

        // Reset the activation grads that aren't around the boundary.
        __syncthreads();
        activationsGradAcc[paddedOffset] = gateValue * outGrad;
        __syncthreads();

        // Accumulate bias/weight grads
        localBiasGradAcc += actDotGrad;
        localGateBiasGradAcc += gateDotGrad;
        for (uint a = 0; a < 3; a++) {
            for (uint b = 0; b < 3; b++) {
                for (uint c = 0; c < 3; c++) {
                    uint cellOffset = threadIdx.x + c + (blockSize + 2) * (threadIdx.y + b + (blockSize + 2) * (threadIdx.z + a));
                    scalar_t act = activations[cellOffset];
                    localWeightsGradAcc[a * 9 + b * 3 + c] += act * actDotGrad;
                    localGateWeightsGradAcc[a * 9 + b * 3 + c] += act * gateDotGrad;
                    scalar_t weight = localWeights[a * 9 + b * 3 + c];
                    atomicAdd(&activationsGradAcc[cellOffset], weight * actDotGrad);
                    scalar_t gateWeight = localGateWeights[a * 9 + b * 3 + c];
                    atomicAdd(&activationsGradAcc[cellOffset], gateWeight * gateDotGrad);
                }
            }
        }
    ...
    for (uint i = 0; i < 27; i++) {
        atomicAdd(
            &weightGradOut[i][0][blockZ * blockSize + threadIdx.z][blockY * blockSize + threadIdx.y][blockX * blockSize + threadIdx.x],
            localWeightsGradAcc[i]);
        atomicAdd(
            &weightGradOut[i][1][blockZ * blockSize + threadIdx.z][blockY * blockSize + threadIdx.y][blockX * blockSize + threadIdx.x],
            localGateWeightsGradAcc[i]);
    }
    atomicAdd(
        &biasGradOut[0][blockZ * blockSize + threadIdx.z][blockY * blockSize + threadIdx.y][blockX * blockSize + threadIdx.x],
        localBiasGradAcc);
    atomicAdd(
        &biasGradOut[1][blockZ * blockSize + threadIdx.z][blockY * blockSize + threadIdx.y][blockX * blockSize + threadIdx.x],
        localGateBiasGradAcc);
}
...
#define GATED_FORWARD_WITH_ACT(act)                                                   \
    AT_DISPATCH_FLOATING_TYPES(                                                       \
        weight.scalar_type(),                                                         \
        "gated_gridnet_cuda_forward",                                                 \
        ([&] { gated_gridnet_cuda_forward_kernel<scalar_t, act><<<blocks, threads>>>( \
                   weight.packed_accessor32<scalar_t, 5>(),                           \
                   bias.packed_accessor32<scalar_t, 4>(),                             \
                   initActivations.packed_accessor32<scalar_t, 4>(),                  \
                   outActivations.packed_accessor32<scalar_t, 4>(),                   \
                   innerIterations); }));
    ...
    std::string &activation)
{
    ...
    // Macro for instantiating many different template variations.
#define GATED_BACKWARD(inner, act) AT_DISPATCH_FLOATING_TYPES(                            \
    weight.scalar_type(),                                                                 \
    "gated_gridnet_cuda_backward",                                                        \
    ([&] { gated_gridnet_cuda_backward_kernel<scalar_t, act, inner><<<blocks, threads>>>( \
               weight.packed_accessor32<scalar_t, 5>(),                                   \
               bias.packed_accessor32<scalar_t, 4>(),                                     \
               initActivations.packed_accessor32<scalar_t, 4>(),                          \
               outGrads.packed_accessor32<scalar_t, 4>(),                                 \
               weightGradOut.packed_accessor32<scalar_t, 5>(),                            \
               biasGradOut.packed_accessor32<scalar_t, 4>(),                              \
               activationsGradOut.packed_accessor32<scalar_t, 4>(),                       \
               innerIterations); }));
...
#define GATED_BACKWARD_WITH_ACT(act)                                                       \
    if (innerIterations <= 8) {                                                            \
        ...
        GATED_BACKWARD(16, act);                                                           \
    } else if (innerIterations <= 32) {                                                    \
        GATED_BACKWARD(32, act);                                                           \
    } else {                                                                               \
        throw std::runtime_error("cannot backprop through more than 32 inner iterations"); \
    }
